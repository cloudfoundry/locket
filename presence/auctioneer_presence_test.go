...
var _ = Describe("AuctioneerPresence", func() {
	...
	BeforeEach(func() {
		auctioneerPresence = presence.NewAuctioneerPresence("some-id", "some-address")
	...
	Describe("ToJSON", func() {
		It("should JSONify", func() {
			json, err := presence.ToJSON(&auctioneerPresence)
	...
	Describe("FromJSON", func() {
		It("returns an AuctioneerPresence with correct fields", func() {
			decodedAuctioneerPresence := &presence.AuctioneerPresence{}
			err := presence.FromJSON([]byte(payload), decodedAuctioneerPresence)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("Validate", func() {
		...
		Context("when ID of auctioneer presense is invalid", func() {
			BeforeEach(func() {
				auctioneerPresence = presence.NewAuctioneerPresence("", "some-address")
			})

			It("returns no error", func() {
				err := auctioneerPresence.Validate()
				Expect(err).To(HaveOccurred())
				Expect(err).To(ContainElement(presence.ErrInvalidField{"auctioneer_id"}))
			})
		})

		Context("when address of auctioneer presense is invalid", func() {
			BeforeEach(func() {
				auctioneerPresence = presence.NewAuctioneerPresence("some-id", "")
			})

			It("returns no error", func() {
				err := auctioneerPresence.Validate()
				Expect(err).To(HaveOccurred())
				Expect(err).To(ContainElement(presence.ErrInvalidField{"auctioneer_address"}))
			})
		})
	})
})
