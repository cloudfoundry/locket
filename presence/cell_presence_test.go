...
var _ = Describe("CellPresence", func() {
	...
	var capacity presence.CellCapacity

	BeforeEach(func() {
		capacity = presence.NewCellCapacity(128, 1024, 3)
		rootfsProviders := []string{"provider-1"}
		preloadedRootFSes := []string{"provider-2"}
		cellPresence = presence.NewCellPresence("some-id", "some-address", "some-zone", capacity, rootfsProviders, preloadedRootFSes)
	...
	Describe("ToJSON", func() {
		It("should JSONify", func() {
			json, err := presence.ToJSON(&cellPresence)
	...
	Describe("FromJSON", func() {
		It("returns a CellPresence with correct fields", func() {
			decodedCellPresence := &presence.CellPresence{}
			err := presence.FromJSON([]byte(payload), decodedCellPresence)
			Expect(err).NotTo(HaveOccurred())
